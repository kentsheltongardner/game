...
export default class Puzzle {
    ...
    loadDefault() {
        ...
        for (let i = 0; i < width; i++) {
            ...
            for (let j = 0; j < height; j++) {
                ...
                const n = j > 0;
                if (e)
                    base.connectionBits |= Directions.Bits[Directions.East];
                if (s)
                    base.connectionBits |= Directions.Bits[Directions.South];
                if (w)
                    base.connectionBits |= Directions.Bits[Directions.West];
                if (n)
                    base.connectionBits |= Directions.Bits[Directions.North];
                if (e && s)
                    base.connectionBits |= Directions.Bits[Directions.Southeast];
                if (w && s)
                    base.connectionBits |= Directions.Bits[Directions.Southwest];
                if (w && n)
                    base.connectionBits |= Directions.Bits[Directions.Northwest];
                if (e && n)
                    base.connectionBits |= Directions.Bits[Directions.Northeast];
    ...
    propagateBaseUpdate(gridX, gridY, newBase) {
        const base = this.baseGrid[gridX][gridY];
    ...
    canBasesConnect(base1, base2) {
        ...
        if (!BaseTypes.Powered[base1.type])
            return true;
        if (base1.power !== base2.power)
            return false;
        if (base1.fixed !== base2.fixed)
            return false;
        return true;
    }
    baseConnected(gridX, gridY, direction) {
        if (!this.contains(gridX, gridY))
            return false;
        const base = this.baseGrid[gridX][gridY];
        const bit = Directions.Bits[direction];
        if ((bit & base.connectionBits) === bit)
            return true;
        return false;
    }
    canBaseConnectionToggle(gridX, gridY, direction) {
        if (!this.contains(gridX, gridY))
            return false;
        const oppositeX = gridX + Directions.X[direction];
        const oppositeY = gridY + Directions.Y[direction];
        if (!this.contains(oppositeX, oppositeY))
            return false;
        const base = this.baseGrid[gridX][gridY];
        const oppositeBase = this.baseGrid[oppositeX][oppositeY];
        if (!this.canBasesConnect(base, oppositeBase))
            return false;
        if (Directions.Orthogonal[direction])
            return true;
        if (!this.contains(gridX, oppositeY))
            return false;
        if (!this.contains(oppositeX, gridY))
            return false;
        const adjacentBase1 = this.baseGrid[gridX][oppositeY];
        const adjacentBase2 = this.baseGrid[oppositeX][gridY];
        if (!this.canBasesConnect(base, adjacentBase1))
            return false;
        if (!this.canBasesConnect(base, adjacentBase2))
            return false;
        return true;
    }
    ...
    connectBase(gridX, gridY, direction) {
        if (!this.canBaseConnectionToggle(gridX, gridY, direction))
            return;
        const base = this.baseGrid[gridX][gridY];
        const bit = Directions.Bits[direction];
        if ((base.connectionBits & bit) === bit)
            return;
    ...
    disconnectBase(gridX, gridY, direction) {
        if (!this.contains(gridX, gridY))
            return;
        const base = this.baseGrid[gridX][gridY];
        const bit = Directions.Bits[direction];
    ...
    // #     #  #     #  #    #   #     #  
    //  #####   #     #  #     #  ######   
    updateCard(gridX, gridY, newTopFace, newBottomFace) {
        if (!this.contains(gridX, gridY))
            return;
    ...
    propagateFaceUpdate(face, newFace) {
        if (face.type === newFace.type && face.watermark === newFace.watermark)
            return;
    ...
    // #     #  #     #  #    #   #     #  
    //  #####   #     #  #     #  ######   
    flipFoldCard(gridX, gridY, dx, dy, strict = false) {
        if (!this.contains(gridX, gridY))
            return;
        if (!this.contains(gridX + dx, gridY + dy))
            return;
        ...
        for (const face of flipFoldSet) {
            if (strict && this.tileGrid[face.x][face.y] !== null)
                return;
            switch (direction) {
                case Directions.East: {
                    ...
                    if (reflectedX > this.width() - 1)
                        return;
                    if (strict && this.tileGrid[reflectedX][face.y] !== null)
                        return;
                    break;
                }
                case Directions.South: {
                    ...
                    if (reflectedY > this.height() - 1)
                        return;
                    if (strict && this.tileGrid[face.x][reflectedY] !== null)
                        return;
                    break;
                }
    ...
    recursiveDeleteCard(gridX, gridY) {
        if (!this.contains(gridX, gridY))
            return;
        if (this.faceGrid[gridX][gridY].length === 0)
            return;
    ...
    propagateRecursiveDeleteFace(face) {
        if (face.flag)
            return;
        face.flag = true;
    ...
    propagateMovement(gridX, gridY, dx, dy, direction, oppositeDirection) {
        const tile = this.tileGrid[gridX][gridY];
        if (tile !== null
            && tile.material !== Materials.Glass)
            return false;
        const face = this.topCardTopFace(gridX, gridY);
        if (face.type !== FaceTypes.Front)
            return false; // Spiders as exception?
        if (face.rule === FaceRules.None)
            return true;
    ...
    //    #        #     #        #        
    //    #     #######  #######  #######  
    validTile(gridX, gridY) {
        if (!this.contains(gridX, gridY))
            return false;
        if (this.tileGrid[gridX][gridY] === null)
            return false;
        return true;
    }
    canTilesConnect(tile1, tile2) {
        if (tile1.material !== tile2.material)
            return false;
        return true;
    }
    tileConnected(gridX, gridY, direction) {
        if (!this.validTile(gridX, gridY))
            return false;
        const tile = this.tileGrid[gridX][gridY];
        const bit = Directions.Bits[direction];
        if ((bit & tile.connectionBits) === bit)
            return true;
        return false;
    }
    canTileConnectionToggle(gridX, gridY, direction) {
        if (!this.validTile(gridX, gridY))
            return false;
        const oppositeX = gridX + Directions.X[direction];
        const oppositeY = gridY + Directions.Y[direction];
        if (!this.validTile(oppositeX, oppositeY))
            return false;
        const tile = this.tileGrid[gridX][gridY];
        const oppositeTile = this.tileGrid[oppositeX][oppositeY];
        if (!this.canTilesConnect(tile, oppositeTile))
            return false;
        if (Directions.Orthogonal[direction])
            return true;
        if (!this.validTile(gridX, oppositeY))
            return false;
        if (!this.validTile(oppositeX, gridY))
            return false;
        const adjacentTile1 = this.tileGrid[gridX][oppositeY];
        const adjacentTile2 = this.tileGrid[oppositeX][gridY];
        if (!this.canTilesConnect(tile, adjacentTile1))
            return false;
        if (!this.canTilesConnect(tile, adjacentTile2))
            return false;
        return true;
    }
    ...
    connectTile(gridX, gridY, direction) {
        if (!this.canTileConnectionToggle(gridX, gridY, direction))
            return;
        const tile = this.tileGrid[gridX][gridY];
        const bit = Directions.Bits[direction];
        if ((tile.connectionBits & bit) === bit)
            return;
    ...
    disconnectTile(gridX, gridY, direction) {
        if (!this.validTile(gridX, gridY))
            return;
        const tile = this.tileGrid[gridX][gridY];
        const bit = Directions.Bits[direction];
    ...
    //    #        #     #        #        
    //    #     #######  #######  #######  
    updateTile(gridX, gridY, newTile) {
        if (!this.validTile(gridX, gridY))
            return;
        this.propagateTileUpdate(gridX, gridY, newTile);
        // TODO once tiles figured out
    }
    propagateTileUpdate(gridX, gridY, newTile) {
        const tile = this.tileGrid[gridX][gridY];
        if (tile.material === newTile.material && tile.fixed === newTile.fixed)
            return;
    ...
    recursiveDeleteTile(gridX, gridY) {
        if (!this.validTile(gridX, gridY))
            return;
        this.propagateTileDelete(gridX, gridY);
    }
    propagateTileDelete(gridX, gridY) {
        const tile = this.tileGrid[gridX][gridY];
        if (tile === null)
            return;
        this.tileGrid[gridX][gridY] = null;
    ...
    // #        #     #  #     #     #     #        #    ##  
    // #        #     #   #####      #     #######  #     #  
    toggleFasteners(gridX, gridY, dx, dy) {
        if (!this.contains(gridX, gridY))
            return;
        const tile = this.tileGrid[gridX][gridY];
        if (tile === null)
        ...
        if ((tile.connectionBits & directionBit) === directionBit)
            return;
        const oppositeDirection = Directions.Opposite[direction];
    ...
    canTileSlide(gridX, gridY, dx, dy) {
        ...
        if (tile.flag)
            return true;
        tile.flag = true;
        if (!this.canTileSlide(gridX + dx, gridY + dy, dx, dy))
            return false;
        for (let i = Directions.East; i <= Directions.North; i++) {
            const bit = Directions.Bits[i];
            const connected = (tile.connectionBits & bit) === bit;
            const fastened = (tile.fastenerBits & bit) === bit;
            if (!connected && !fastened)
                continue;
            const connectionX = gridX + Directions.X[i];
            const connectionY = gridY + Directions.Y[i];
            if (!this.canTileSlide(connectionX, connectionY, dx, dy))
                return false;
        }
        return true;
    }
    slideTiles(gridX, gridY, dx, dy) {
        if (!this.contains(gridX, gridY))
            return;
        const tile = this.tileGrid[gridX][gridY];
        if (tile === null)
