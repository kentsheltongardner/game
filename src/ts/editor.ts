...
export default class Editor {
    ...
    public controlSelection = new Point(0, 0)
    public mouseData        = new MouseData()
    ...
    populateNumericSelect(selectId: string, count: number) {
        const select = <HTMLSelectElement>document.getElementById(selectId)
    ...
    mouseWheel(e: WheelEvent) {
        if (e.deltaY > 0) {
            this.camera.zoomOut(
                this.mouseData.canvasX, 
                this.mouseData.canvasY,
                this.canvas.width,
                this.canvas.height
            )
        } else {
    ...
    // Create
    // Select
    //      Edit/control selection - copy selection (many)
    // Edit
    // Connect

    // If one face connected, both faces must connect
}
